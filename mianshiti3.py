...
#=======bfs.但是太慢了.
tmp=[[i] for i in range(len(all_combine))]
for i in all_combine:
    #=====至多进行这么多次组合
    next_step=[]
    #=====改成用在all_combine中的索引来表示组合.
    for j in tmp:
        for test_add in range(j[-1]+1,len(all_combine)): #尝试加入比他大的索引的括号.
            #判断test_add是否能加入j里面.
            if panding2([all_combine[x] for x in j],all_combine[test_add]):
...
#=========
out=tmp[0]
out=[all_combine[i] for i in out]
out2='*'*len(shuru)
out2=list(out2)
for i in out:
    out2[i[0]]='('
